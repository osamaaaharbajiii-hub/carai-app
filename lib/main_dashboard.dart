...
class _MainDashboardState extends State<MainDashboard> {
  ...
  void initState() {
    super.initState();
    selectedProtocol = defaultProtocols[0];
  }
  ...
  Future<void> _connectToDevice(BluetoothDevice device) async {
    try {
      ...
      final service = ObdService(device);
      bool initialized = await service.initialize();

      if (initialized) {
        await service.setupAdapter();
        _obdService = service;
      }
              ...
              children: [
                ...
                Expanded(
                  child: ListView.builder(
                    shrinkWrap: true,
                    itemCount: defaultProtocols.length,
                    itemBuilder: (context, index) {
                      final item = defaultProtocols[index];
                      bool isFirstOfCategory = index == 0 ||
                          defaultProtocols[index - 1].category != item.category;
  ...
  void _runFullDiagnostic() async {
    if (!isConnected || _obdService == null) {
      _openToolModal('DTC Scanner', const Text('Please connect to an OBD-II adapter first.'));
      return;
    }

    _openToolModal('DTC Scanner', FutureBuilder<List<String>>(
      future: _obdService!.readDtcCodes(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Column(
            mainAxisSize: MainAxisSize.min,
            children: const [
              ...
              SizedBox(height: 15),
              Text('Scanning ECU for fault codes...'),
            ],
          );
        }
        if (snapshot.hasError) {
          return Text('Error scanning DTCs: ${snapshot.error}');
        }
        final codes = snapshot.data ?? [];
        if (codes.isEmpty) {
          return const Text('No Diagnostic Trouble Codes (DTCs) found. ECU is clear!');
        }
        return Column(
          mainAxisSize: MainAxisSize.min,
          children: codes.map((code) => ListTile(
        ...
        child: Column(
          ...
          children: [
            ...
            GridView.count(
              ...
              mainAxisSpacing: 12,
              children: [
                _buildToolCard('Full Diagnostic', Icons.troubleshoot, Colors.amber, _runFullDiagnostic),
